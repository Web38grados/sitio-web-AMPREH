      ...
      >
        ...
        <div className="bg-white shadow-[0_4px_20px_-10px_rgba(0,0,0,0.1)]">
          <div className="mx-auto flex max-w-7xl items-center justify-between px-5 lg:px-8 h-20">
            ...
            <button type="button" onClick={() => setOpen(!open)} className="rounded-none p-2 text-[#004a99] transition-colors hover:bg-slate-100 lg:hidden border border-slate-200">
              {open ? <X /> : <Menu />}
            </button>
            
            <nav className={`${open ? 'flex' : 'hidden'} absolute inset-x-0 top-full flex-col border-b-4 border-[#004a99] bg-white p-5 shadow-2xl lg:static lg:flex lg:flex-row lg:items-center lg:justify-end lg:flex-grow lg:gap-8 lg:border-0 lg:bg-transparent lg:p-0 lg:shadow-none`}>
                  ...
                  return (
                    <Link
                      ...
                      className={`group relative px-4 py-3 text-[13px] font-black uppercase tracking-widest transition-colors ${
                        isActive ? 'text-[#004a99]' : 'text-slate-600 hover:text-[#004a99]'
                      }`}
                    >
                      {label}
              ...
              <Link to="/contacto" onClick={() => setOpen(false)} className="mt-4 lg:mt-0 lg:ml-6 flex items-center justify-center gap-2 bg-[#F58220] px-6 py-3.5 text-[11px] font-black uppercase tracking-widest text-white transition-all hover:bg-[#004a99] shadow-md hover:shadow-lg rounded-none">
